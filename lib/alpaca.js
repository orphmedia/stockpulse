...
/**
 * Yahoo Finance real-time quotes (requires cookie + crumb auth)
 */
...
async function getYahooCrumb() {
  if (_yahooCrumb && _yahooCookie && Date.now() < _yahooCrumbExpiry) {
  ...
  _yahooCrumb = crumb;
  _yahooCookie = cookie;
  _yahooCrumbExpiry = Date.now() + 300000; // cache 5 min
  return { crumb, cookie };
}
...
export async function getYahooQuotes(symbols) {
  try {
    ...
    const res = await fetch(
      `https://query2.finance.yahoo.com/v7/finance/quote?symbols=${symbolStr}&crumb=${encodeURIComponent(crumb)}&fields=regularMarketPrice,regularMarketChange,regularMarketChangePercent,shortName,regularMarketPreviousClose,regularMarketVolume,regularMarketDayHigh,regularMarketDayLow,regularMarketOpen,trailingAnnualDividendRate,trailingAnnualDividendYield,dividendDate,epsTrailingTwelveMonths,trailingPE,marketCap,fiftyTwoWeekHigh,fiftyTwoWeekLow,averageAnalystRating,targetMeanPrice,targetHighPrice,targetLowPrice,numberOfAnalystOpinions,recommendationMean,recommendationKey`,
    ...
    for (const q of data.quoteResponse?.result || []) {
      if (q.regularMarketPrice) {
        results[q.symbol] = {
          ...
          targetHigh: q.targetHighPrice || 0,
          targetLow: q.targetLowPrice || 0,
          numAnalysts: q.numberOfAnalystOpinions || 0,
          recommendationMean: q.recommendationMean || 0,
